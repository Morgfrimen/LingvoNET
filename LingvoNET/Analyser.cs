...
public static class Analyser
{
    ...
    private static readonly List<WordForm> Items = new();
    ...
    public static IEnumerable<WordForm> FindAllSourceForm(string form)
    {
        ...
        return Items.FindAll(wordForm, new StringReverseComparer<WordForm>());
    }
    ...
    public static WordForm FindSimilarSourceForm(string form, Predicate<WordForm> filter = null)
    {
        if (filter == null)
            filter = w => true;
        var wordForm = new WordForm { Form = form };
        var res = Items.FindSimilar(wordForm, new StringReverseComparer<WordForm>(), filter);
    ...
    private static void GetDifferenceReverse(ref string s1, ref string s2)
    {
        var i = 0;
